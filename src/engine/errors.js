...
export function createUnsupportedFeatureError(feature, position = null) {
  return new SqlError(
    ...
    `Unsupported feature: ${feature}. Tip: this simulator supports SELECT, FROM, INNER JOIN, WHERE (AND, OR, NOT, IN, BETWEEN), GROUP BY, HAVING, ORDER BY, LIMIT, DISTINCT, basic aggregates, subqueries, and simple DDL/DML. Remove '${feature}' or replace it with a supported construct.`,
    position
  );
}
